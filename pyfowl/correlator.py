...
class Correlator(object):
    ...
    def __init__(self, theta, z, nlens, nsource, limber=False, limber_2=False, limber_gs=False, shear_beyond_limber=False, curved_sky=False, halofit=False,
        rsd=False, mag=False, baryons=False, nnlo=False, photoz=False, zlens_mean=None, intrinsic_alignments=False, shear_calibration=False,
        binning=False, theta_min=None, theta_max=None, 
        fftpad=False, noloop=False, 
        extended=True, load=True, save=True, path='./', NFFT=256, Nsum=400, knl=0.7, km=0.7):
        
        self.limber = limber
        self.limber_2 = limber_2
        self.limber_gs = limber_gs
        self.shear_beyond_limber = shear_beyond_limber
        self.curved_sky = curved_sky
        self.halofit = halofit # halofit only for linear power spectra
        self.rsd = rsd
        self.mag = mag
        self.baryons = baryons
        self.photoz = photoz
        self.intrinsic_alignments = intrinsic_alignments
        ...
        self.theta_bin = theta
        self.binning = binning
        if self.binning: 
            self.binvol = np.array([np.cos(tmax) - np.cos(tmin) for (tmin, tmax) in zip(theta_min, theta_max)]) 
            self.binpts = [np.linspace(np.cos(tmin), np.cos(tmax), 30) for (tmin, tmax) in zip(theta_min, theta_max)] # tested 30 vs 100
            inv_theta_thin = np.geomspace(1./np.max(theta_max), 1./np.min(theta_min), 60) # we want to have a lot of points at large theta where the binning effect is large
            self.theta_thin = np.sort(1./inv_theta_thin) # np.linspace(np.min(theta_min), np.max(theta_max), 2000) # tested against 2000-linspaced points
        else:
            self.theta_thin = theta
        self.theta, _ = np.meshgrid(self.theta_thin, self.z_thin_arr, indexing='ij')
        ...
        if self.extended: self.Nmarg = self.Nbin+2*self.Ng # Nbin [counterterms] + 2*Ng [b3] (two independent sets of (correlated) galaxy biases for gg and gs)
        else: self.Nmarg = self.Nbin+self.Ng # Nbin [counterterms] + Ng [b3]
        
        if fftpad: self.fftextrap = 'padding'
        else: self.fftextrap = 'extrap'
        ...
        if load is True:
            try:
                ...
                if (self.fft0.Pow - L['Pow0']).any() or (self.fft1.Pow - L['Pow1']).any() or (self.fft2.Pow - L['Pow2']).any():
                    ...
                    self.M0, self.M22, self.M13, self.W2, self.W22, self.W13, self.Mrsd = L['M0'], L['M22'], L['M13'], L['W2'], L['W22'], L['W13'], L['Mrsd']
                    save = False
            except:
        ...
        if save is True:
            try: np.savez(self.pyegg, Pow0=self.fft0.Pow, Pow1=self.fft1.Pow, Pow2=self.fft2.Pow, 
                M0=self.M0, M22=self.M22, M13=self.M13, W2=self.W2, W22=self.W22, W13=self.W13, Mrsd=self.Mrsd)
    ...
    def _load_wigner(self): 
        self.ls = np.arange(1, 3500, 1)
        mu = np.cos(self.theta_thin)
        def eval_legendre2(l, x): return l * (l - 1) * (1 - x**2) * eval_legendre(l - 2, x) # associated legendre of order m = 2
        self.leg = np.array([[eval_legendre(ell, m) for m in mu] for ell in self.ls])
        self.leg2 = np.array([[float(eval_legendre2(ell, m)) for m in mu] for ell in self.ls])
        def Wigner_d_ss(ell, mu, i, j, sign='+'): # (A4) and (A5) of https://arxiv.org/pdf/2012.08568.pdf
            if sign == '+': s = +1
            elif sign == '-': s = -1
            return (2*ell+1)/(4.*np.pi*ell**2*(ell+1)**2) * 2 * ( self.leg2[i,j] * ( (4-ell + s*2*mu*(ell-1))/(1.-mu**2) - ell*(ell-1)/2. ) + self.leg2[i-1,j] * (ell+2)*(mu-s*2)/(1.-mu**2) )
        self.wig_d_gg = np.array([[(2*ell+1)/(4.*np.pi) * self.leg[i,j] for j, m in enumerate(mu)] for i, ell in enumerate(self.ls)])
        self.wig_d_gs = np.array([[(2*ell+1)/(4.*np.pi) * self.leg[i,j] for j, m in enumerate(mu)] for i, ell in enumerate(self.ls)])
        self.wig_d_ssp = np.array([[Wigner_d_ss(ell, m, i, j, sign='+') for j, m in enumerate(mu)] for i, ell in enumerate(self.ls)])
        self.wig_d_ssm = np.array([[Wigner_d_ss(ell, m, i, j, sign='-') for j, m in enumerate(mu)] for i, ell in enumerate(self.ls)])
        self.wig_d_ssp[0,:] *= 0. # setting ell = 1 contributions to 0
        self.wig_d_ssm[0,:] *= 0. # setting ell = 1 contributions to 0
        return
    ...
    def _get_cls_limber(self, t1, t2, ls, d1=0, d2=0, halofit=False, loop_scaling_1=False, loop_scaling_2=False):
        """
        Attributes
        ----------
        d1, d2 : 0, 1, 2
        ...
        if halofit: pk = np.exp(self.pknl_interp((np.log(k), self.chi_logspace_arr)))
        else: pk = np.exp(self.pklin_interp(np.log(k)))
        if loop_scaling_1: pk *= k**2/self.knl**2/(1+k**2/self.knl**2) # using Pade for stability in the cls projection; putting only counterterm scaling since > pt scaling;
        elif loop_scaling_2: pk *= k**4/self.knl**4/(1+k**4/self.knl**4) 
        cls_limber = np.trapz(get_transfer(t1, ell, d=d1) * get_transfer(t2, ell, d=d2) / self.chi_logspace_arr**2 * pk, x=self.chi_logspace_arr, axis=-1)
        return cls_limber
    
    def _get_cls_nonlimber(self, fchi1, fchi2, ls, d1=0, d2=0, loop_scaling_1=False, loop_scaling_2=False):
        nu = 1.01
        ...
        def get_fftlog(myfftlog, ell, d=0):
            if d == 0: k, fk = myfftlog.fftlog(ell)
                ...
                fk *= ell*(ell+1)/k**2 
            elif d == 2: k, fk = myfftlog.fftlog_ddj(ell) # rsd
            return k, fk
        for i in np.arange(Nell):
            ell = ls[i]
            k, fk1 = get_fftlog(myfftlog1, ell, d=d1)
            k, fk2 = get_fftlog(myfftlog2, ell, d=d2)
            pk = np.exp(self.pklin_interp(np.log(k)))
            if loop_scaling_1: pk *= k**2/self.knl**2/(1+k**2/self.knl**2) # using Pade for stability in the cls projection; putting only counterterm scaling since > pt scaling;
            elif loop_scaling_2: pk *= k**4/self.knl**4/(1+k**4/self.knl**4)
            cls_nonlimber[i] = np.sum(fk1 * fk2 * k**3 * pk) * self.dlnr * 2./np.pi
        return cls_nonlimber
    
    def _get_cls(self, t1, t2, d1=0, d2=0, l_split=400, limber=False, halofit=False, loop_scaling_1=False, loop_scaling_2=False):
        if self.limber_2 or limber: 
            cls_limber = self._get_cls_limber(t1, t2, self.ls, d1=d1, d2=d2, halofit=halofit, loop_scaling_1=loop_scaling_1, loop_scaling_2=loop_scaling_2) 
            return cls_limber
        else:
            cls_nonlimber = self._get_cls_nonlimber(t1, t2, self.ls[self.ls<l_split], d1=d1, d2=d2, loop_scaling_1=loop_scaling_1, loop_scaling_2=loop_scaling_2)
            cls_limber = self._get_cls_limber(t1, t2, self.ls[self.ls>=l_split], d1=d1, d2=d2, loop_scaling_1=loop_scaling_1, loop_scaling_2=loop_scaling_2)
            return np.hstack((cls_nonlimber, cls_limber)) 
    
    def _get_xi(self, cl, cl_type='gg', curved_sky=False):
        if self.curved_sky or curved_sky: 
            if 'gg' in cl_type: wig_d = self.wig_d_gg
            elif 'gs' in cl_type: wig_d = self.wig_d_gs
            elif 'ssp' in cl_type: wig_d = self.wig_d_ssp
            elif 'ssm' in cl_type: wig_d = self.wig_d_ssm
            return np.einsum('l,lt->t', cl, wig_d)
        else:
            # Coef = self.fft.Coef(self.ls, cl, extrap='extrap', window=None)
            Coef = self.fft.Coef(self.ls, cl, extrap='padding', window=None) # we 0-pad instead of extrap given that sometimes cl is negative for e.g., some rsd x mag 
    ...
    def _get_xi_loop_scaling(self, limber=True, b1=2.):
        self.xi_gg_1loop_scaling = 2*b1 * np.array([self._get_xi(self._get_cls(t1, t1, limber=limber, loop_scaling_1=True), cl_type='gg') for t1 in self.tgal])
        self.xi_gs_1loop_scaling = (b1 + 1) * np.array([self._get_xi(self._get_cls(t1, t2, limber=limber, loop_scaling_1=True), cl_type='gs') for t1 in self.tgal for t2 in self.tshear])
        self.xi_ssp_1loop_scaling = 2*2 * np.array([self._get_xi(self._get_cls(t1, t2, limber=limber, loop_scaling_1=True), cl_type='ssp') for i, t1 in enumerate(self.tshear) for j, t2 in enumerate(self.tshear) if i <= j]) # c_ss ~ 2 as measured from halofit, see prior_mean_xssp
        self.xi_ssm_1loop_scaling = 2*2 * np.array([self._get_xi(self._get_cls(t1, t2, limber=limber, loop_scaling_1=True), cl_type='ssm') for i, t1 in enumerate(self.tshear) for j, t2 in enumerate(self.tshear) if i <= j])

        self.xi_gg_2loop_scaling = 4 * np.array([self._get_xi(self._get_cls(t1, t1, limber=limber, loop_scaling_2=True), cl_type='gg') for t1 in self.tgal])
        self.xi_gs_2loop_scaling = (b1 + 1)**2/b1 * np.array([self._get_xi(self._get_cls(t1, t2, limber=limber, loop_scaling_2=True), cl_type='gs') for t1 in self.tgal for t2 in self.tshear])
        self.xi_ssp_2loop_scaling = 4 * np.array([self._get_xi(self._get_cls(t1, t2, limber=limber, loop_scaling_2=True), cl_type='ssp') for i, t1 in enumerate(self.tshear) for j, t2 in enumerate(self.tshear) if i <= j])
        self.xi_ssm_2loop_scaling = 4 * np.array([self._get_xi(self._get_cls(t1, t2, limber=limber, loop_scaling_2=True), cl_type='ssm') for i, t1 in enumerate(self.tshear) for j, t2 in enumerate(self.tshear) if i <= j])
    ...
    def _get_xi_lin_nonlimber(self):            
        ...
        self.xi_gs_lin_nonlimber = np.array([self._get_xi(self._get_cls(t1, t2, limber=self.limber_gs), cl_type='gs') for t1 in self.tgal for t2 in self.tshear])
        if self.shear_beyond_limber:
        ...
        if self.halofit: 
            self.xi_ssp_lin_nonlimber = np.array([self._get_xi(self._get_cls(t1, t2, limber=True, halofit=True), cl_type='ssp') for i, t1 in enumerate(self.tshear_noDz) for j, t2 in enumerate(self.tshear_noDz) if i <= j])
            self.xi_ssm_lin_nonlimber = np.array([self._get_xi(self._get_cls(t1, t2, limber=True, halofit=True), cl_type='ssm') for i, t1 in enumerate(self.tshear_noDz) for j, t2 in enumerate(self.tshear_noDz) if i <= j])
        if self.rsd: 
            ...
            self.xi_gs_lin_rsd_nonlimber = np.array([-1. * self._get_xi(-1. * self._get_cls(t1, t2, d1=2, d2=0, limber=self.limber_gs), cl_type='gs') for t1 in self.tgal_rsd for t2 in self.tshear]) # -1 x -1 trick since cls_rsd_shear is negative in the limits ell -> 0 or ell -> infty (we need positive cls there for safe loglog-extrapolation, see in fftlog)
        if self.mag:
            ...
            self.xi_gs_lin_mag_nonlimber = np.array([self._get_xi(self._get_cls(t1, t2, d1=1, d2=0, limber=self.limber_gs), cl_type='gs') for t1 in self.tgal_mag for t2 in self.tshear])
        if self.rsd and self.mag: 
            self.xi_gg_lin_rsd_mag_nonlimber = np.array([-1. * self._get_xi(-1. * self._get_cls(t1, t2, d1=2, d2=1), cl_type='gg') for t1, t2 in zip(self.tgal_rsd, self.tgal_mag)]) # -1 x -1 trick since cls_rsd_mag is negative in the limits ell -> 0 or ell -> infty (we need positive cls there for safe loglog-extrapolation, see in fftlog)

    def _get_cls_lin_nonlimber(self):            
        self.cls_gg_lin_nonlimber = np.array([self._get_cls(t1, t1) for t1 in self.tgal])
        self.cls_gs_lin_nonlimber = np.array([self._get_cls(t1, t2, limber=self.limber_gs) for t1 in self.tgal for t2 in self.tshear])
        if self.shear_beyond_limber:
            self.xi_ssp_lin_nonlimber = np.array([self._get_cls(t1, t2) for i, t1 in enumerate(self.tshear) for j, t2 in enumerate(self.tshear) if i <= j])
            self.xi_ssm_lin_nonlimber = np.array([self._get_cls(t1, t2) for i, t1 in enumerate(self.tshear) for j, t2 in enumerate(self.tshear) if i <= j])
        if self.halofit: 
            self.xi_ssp_lin_nonlimber = np.array([self._get_cls(t1, t2, limber=True, halofit=True) for i, t1 in enumerate(self.tshear_noDz) for j, t2 in enumerate(self.tshear_noDz) if i <= j])
            self.xi_ssm_lin_nonlimber = np.array([self._get_cls(t1, t2, limber=True, halofit=True) for i, t1 in enumerate(self.tshear_noDz) for j, t2 in enumerate(self.tshear_noDz) if i <= j])
        if self.rsd: 
            self.cls_gg_lin_rsd_nonlimber = np.array([
                [self._get_cls(t1, t2, d1=0, d2=2) for t1, t2 in zip(self.tgal, self.tgal_rsd)], 
                [self._get_cls(t1, t1, d1=2, d2=2) for t1 in self.tgal_rsd], 
                    ])
            self.cls_gs_lin_rsd_nonlimber = np.array([self._get_cls(t1, t2, d1=2, d2=0, limber=self.limber_gs) for t1 in self.tgal_rsd for t2 in self.tshear]) 
        if self.mag:
            self.cls_gg_lin_mag_nonlimber = np.array([
                [self._get_cls(t1, t2, d1=0, d2=1) for t1, t2 in zip(self.tgal, self.tgal_mag)], 
                [self._get_cls(t1, t1, d1=1, d2=1) for t1 in self.tgal_mag], 
                    ])
            self.cls_gs_lin_mag_nonlimber = np.array([self._get_cls(t1, t2, d1=1, d2=0, limber=self.limber_gs) for t1 in self.tgal_mag for t2 in self.tshear])
        if self.rsd and self.mag: 
            self.cls_gg_lin_rsd_mag_nonlimber = np.array([self._get_cls(t1, t2, d1=2, d2=1) for t1, t2 in zip(self.tgal_rsd, self.tgal_mag)]) 
    ...
    def get_bessel_transforms(self, kin, Pin, window=None):
        ...
        if self.fftextrap == 'extrap': # I can't remember why we put a kdeep since the FFTLog will anyway extrapolate automatically if needed? Is it for the Pade of counterterm?
            Pk_interp = interp1d(np.log(kin), np.log(Pin), fill_value='extrapolate')
            Pk = np.exp(Pk_interp(np.log(self.kdeep)))
        elif self.fftextrap == 'padding':
            Pk = interp1d(kin, Pin, kind='cubic', bounds_error=False, fill_value=0.)(self.kdeep)

        coef11 = self.fft0.Coef(self.kdeep, Pk, extrap=self.fftextrap, window=.2)
        coefsPow11 = np.einsum('n,ns->ns', coef11, self.sPow)
        coefct = self.fft0.Coef(self.kdeep, self.kdeep**2/(1.+self.kdeep**2) * Pk, extrap=self.fftextrap, window=.2)
        coefsPowct = np.einsum('n,ns->ns', coefct, self.sPow)
        coef1 = self.fft1.Coef(self.kdeep, Pk, extrap=self.fftextrap, window=.2)
    ...
    def get_bessel_transforms_rsd(self, kin, Pin, window=None):
        ...
        if self.fftextrap == 'extrap':
            Pk_interp = interp1d(np.log(kin), np.log(Pin), fill_value='extrapolate')
            Pk = np.exp(Pk_interp(np.log(self.kdeep)))
        elif self.fftextrap == 'padding':
            Pk = interp1d(kin, Pin, kind='cubic', bounds_error=False, fill_value=0.)(self.kdeep)

        coef = self.fft0.Coef(self.kdeep, Pk, extrap=self.fftextrap, window=.2)
    ...
    def get_bessel_transforms_baryons(self, kin, Pai, window=None):
        ...
        coef11b = self.fft0.Coef(kin, Pai, extrap=self.fftextrap, window=.2)
    ...
    def get_bessel_transforms_nnlo(self, kin, Pin, window=None):
        ...
        coefnnlo = self.fft0.Coef(self.kdeep, self.kdeep**4/(1.+self.kdeep**4) * Pk, extrap=self.fftextrap, window=.2)
        CoefsPownnlo = np.einsum( 'n,ns->ns', coefnnlo, self.sPow )
        coef2 = self.fft2.Coef(self.kdeep, 1/(1.+self.kdeep) * Pk, extrap=self.fftextrap, window=.2)
    ...
    def compute(self, kin, pk_l, pk_nl, rz, dz_by_dr, Dz, Dfid, fz, ffid, h, Omega0_m, Pai=None, dzs=None, dzl=None, szl=None, A=0., alpha=1.):
        
        self._set_time_kernels(kin, pk_l, pk_nl, rz, dz_by_dr, Dz, Dfid, fz, ffid, h, Omega0_m, dzs=dzs, dzl=dzl, szl=szl, A=A, alpha=alpha)
        ...
        neat_indent = True
        if neat_indent:
            A11, Act, A13, A22 = self.get_bessel_transforms(kin, pk_l) 
        ...
        if self.limber: 
            if self.rsd or self.mag: 
                Add, Adr, Arr = self.get_bessel_transforms_rsd(kin, pk_l) 
                
            if self.rsd: 
                self.Agg_rsd = time_integral(self.qqrsd, self.Dp2, np.array([Adr, Arr]))
                # self.Ags_rsd =  # not coded since we use beyond Limber formula

            if self.mag:
                Amm = time_integral(self.qmqm, self.Dp2, Add) # -1/2. * time_integral(self.qmqm, self.Dp2 * self.rz**-2, Adr) + 1/16. * time_integral(self.qmqm, self.Dp2 * self.rz**-4, Arr)
                Agm = time_integral(self.qgqm, self.Dp2, Add) # -1/4. * time_integral(self.qgqm, self.Dp2 * self.rz**-2, Adr)
                self.Agg_mag = np.array([Agm, Amm])
                # self.Ags_mag =   # not coded since we use beyond Limber formula

            if self.rsd and self.mag:
                pass # as rsd and mag have been checked independently, we don't code up this Limber as we use the beyond Limber formula
        ...
        if not self.limber:
            ...
            if self.shear_beyond_limber:
                self.Assp[0] = self.xi_ssp_lin_nonlimber
                self.Assm[0] = self.xi_ssm_lin_nonlimber
            if self.halofit: 
                self.Assp[0] = self.xi_ssp_lin_nonlimber # limber, but halofit
                self.Assm[0] = self.xi_ssm_lin_nonlimber # limber, but halofit
            if self.rsd: 
                self.Agg_rsd = self.xi_gg_lin_rsd_nonlimber
                self.Ags_rsd = self.xi_gs_lin_rsd_nonlimber
            if self.mag: 
                self.Agg_mag = self.xi_gg_lin_mag_nonlimber
                self.Ags_mag = self.xi_gs_lin_mag_nonlimber 
            if self.rsd and self.mag: 
                self.Agg_rsd_mag = self.xi_gg_lin_rsd_mag_nonlimber
        ...
        if self.nnlo: 
            Vct, V13, V22 = self.get_bessel_transforms_nnlo(kin, pk_l)
        ...
        if self.binning:
            def bin_(X):
                iX = interp1d(np.cos(self.theta_thin), X, axis=-1, kind='cubic')
                res = np.array([np.trapz(iX(pts), x=pts, axis=-1) for pts in self.binpts]) 
                return np.moveaxis(res, 0, -1) / self.binvol

            self.Assp, self.Assm, self.Ags, self.Agg = bin_(self.Assp), bin_(self.Assm), bin_(self.Ags), bin_(self.Agg)
            if self.rsd: self.Agg_rsd, self.Ags_rsd = bin_(self.Agg_rsd), bin_(self.Ags_rsd)
            if self.mag: self.Agg_mag, self.Ags_mag = bin_(self.Agg_mag), bin_(self.Ags_mag)
            if self.rsd and self.mag: self.Agg_rsd_mag = bin_(self.Agg_rsd_mag)
            if self.baryons: self.B11 = bin_(self.B11)
            if self.nnlo: self.Vct, self.Vssp, self.Vssm, self.Vgs, self.Vgg = bin_(self.Vct), bin_(self.Vssp), bin_(self.Vssm), bin_(self.Vgs), bin_(self.Vgg)

        self.Assp_ = 1. * self.Assp
        self.Assm_ = 1. * self.Assm

    def _set_time_kernels(self, kin, pk_l, ilogpk_nl, rz, dz_by_dr, Dz, Dfid, fz, ffid, h, Omega0_m, dzs=None, dzl=None, szl=None, A=0., alpha=1.):
        ...
        lensing_factor = 1.5/conts.c**2 * h**2 * 1e10 * Omega0_m # unit 1/Mpc^2, 1e10 = (1e2)^2/(1e-3)^2 from little h and conversion of c in [m/s] -> [km/s]
        self.r1, _ = np.meshgrid(self.rz, self.z_thin_arr, indexing='ij')
        
        def lensing_efficiency(nz): # unitless ; outside the integral: rz is chi, inside the integral: r1 is chi, rz is chi tilde ; integral is over z, since nz is distributed in redshift, so nz dz = nr dr/dz dz = nr dr
        ...
        if self.photoz:
            nsource = np.array([interp(self.z_thin_arr, self.nsource[i])(self.z_thin_arr-dz) for i, dz in enumerate(dzs)]) 
            nlens = np.array([ sz**-1 * interp(self.z_thin_arr, self.nlens[i])(sz**-1 * (self.z_thin_arr-zm-dz) + zm) for i, (dz,sz,zm) in enumerate(zip(dzl,szl,self.zlens_mean))]) # Eq. (14) of 2012.12826 + Fig. 9 is convincing ; Eq. (14-15) of 2105.13549 are confusing
            # DESY3 collab dz shift ok: https://github.com/joezuntz/cosmosis-standard-library/blob/main/number_density/photoz_bias/photoz_bias.py
            # DESY3 collab sz stretch ok: https://github.com/joezuntz/cosmosis-standard-library/blob/main/number_density/photoz_width/photoz_width.py , as they put the strech inside the definition of the interpolated function, then z = stretch * (z'-zmean)+zmean [passive] <->  z' = (z-zmean)/strech + zmean [active] !
            # however DESY3 first stretch, then shift (https://github.com/joezuntz/cosmosis-standard-library/blob/main/examples/des-y3.ini), which is n_theo(z') = n_true(z') = n_pz((z-zmean)/strech + zmean - dz) 
            # Instead we do Eq. (14) of 2012.12826: n_theo(z') = n_true(z') = n_pz((z-zmean-dz)/strech + zmean)
            # notice that the normalization of nz is still one as shifting it does not change, and we put the correct 1/sz factor if front to keep the normalization constant when stretching
        else: 
            nsource = self.nsource
            nlens = self.nlens

        qgal = np.array([self.dz_by_dr * nl for nl in nlens])
        qshear = np.array([lensing_efficiency(ns) for ns in nsource])
        if self.intrinsic_alignments: qshear += np.array([intrinsic_alignments(A, alpha)*self.dz_by_dr*ns for ns in nsource])

        ### line-of-sight kernels (unitless)
        ...
        if self.mag:
            qmag = np.array([lensing_efficiency(nl) for nl in nlens]) 
            self.qmqm = np.array([qi**2 for qi in qmag])
            self.qgqm = np.array([qi*qj for qi, qj in zip(qgal, qmag)])
            # self.qsqm = np.array([qi*qj for qi in qmag for qj in qshear]) # not coded up since we beyond Limber

        if not self.limber:
            self.pklin_interp = interp1d(np.log(kin), np.log(pk_l), fill_value='extrapolate')
            ...
            if self.halofit: 
                self.pknl_interp = ilogpk_nl
                self.tshear_noDz = interp1d(self.rz, qshear, kind='cubic', fill_value='extrapolate')(self.chi_logspace_arr) # extrapolate to prevent rounding errors
            ...
            if self.mag: self.tgal_mag = interp1d(self.rz, qmag  / self.rz**2 * self.Dz / Dfid, kind='cubic', fill_value='extrapolate')(self.chi_logspace_arr) # 1/chi^2 here!

    def do_shear_calibration(self, ms):
        self.msms = np.array([(1.+mi)*(1.+mj) for i, mi in enumerate(ms) for j, mj in enumerate(ms) if i <= j]) 
        self.Assp = np.swapaxes(np.swapaxes(self.Assp_, -2, -1) * self.msms, -1, -2) # multiplying along axis = -2 (id of bin ij) 
        self.Assm = np.swapaxes(np.swapaxes(self.Assm_, -2, -1) * self.msms, -1, -2) # multiplying along axis = -2 (id of bin ij) 

    def format_bias(self, bng, bg=None, marg=False):

        # order bng: b1, c2, bmag, b1i, bnnlo
        # order bg: cssp, cssm, cgs, cgg, b3
        # cssp/cssm: (j,i) where i,j = 1, ..., Ns with i <=j
        # cgs: (j,i) where i = 1, ..., Ng and j = 1, ..., Ns

        if bg is None: 
            if marg: bg = np.zeros(shape=(self.Nmarg))
            else:
                pad = 2*self.Ng # (b1, c2)
                if self.extended: pad += 2*self.Ng # independent set of (b1, c2) for gs 
                if self.mag: pad += self.Ng 
                if self.baryons: pad += self.Ng 
                if self.nnlo: pad += 4 # gg, gs, ss+, ss-
                if self.photoz: pad += self.Ns + 2 * self.Ng 
                if self.intrinsic_alignments: pad += 2 # A, alpha
                if self.shear_calibration: pad += self.Ns
                bg = bng[pad:] 
        ...
        if self.extended: # independent set of (b1, c2, b3) for gs 
            bgs = np.vstack([[bng[i] for i in np.arange(2*self.Ngg, 3*self.Ngg)], 
                             [bng[i] for i in np.arange(3*self.Ngg, 4*self.Ngg)], 
                             [bg[i] for i in np.arange(self.Nbin+self.Ngg, self.Nbin+2*self.Ngg)], 
                             [bng[i] for i in np.arange(3*self.Ngg, 4*self.Ngg)]]) 
        else: 
            bgs = bgg 

        id_0 = 2*self.Ng # (b1, c2)
        if self.extended: id_0 += 2*self.Ng # independent set of (b1, c2) for gs 
        if self.mag: 
            pad = self.Ng; bmag = bng[id_0:id_0+pad]; id_0 += pad # magnification biases
        else: bmag = np.zeros(shape=(self.Ng)) # dummy
        if self.baryons: 
            pad = self.Ng; b1i  = bng[id_0:id_0+pad]; id_0 += pad # isocurvature linear biases
        else: b1i = np.zeros(shape=(self.Ng)) # dummy
        if self.nnlo: 
            pad = 4; bnnlo = bng[id_0:id_0+pad]; id_0 += pad # nnlo biases
        else: bnnlo = np.zeros(shape=(4)) # dummy

        return bgg, cgg, bgs, cgs, cssp, cssm, bmag, b1i, bnnlo

    def print_bias(self, bval, bg=None, marg=False):
        bgg, cgg, bgs, cgs, cssp, cssm, bmag, b1i, bnnlo = self.format_bias(bval, bg, marg=marg)
        with np.printoptions(precision=3, suppress=True): 
            for i, (b, tag) in enumerate(zip(bgg, ['b1', 'b2', 'b3'])): print (tag, b)
            print ('cgg', cgg)
            if self.extended: 
                for i, (b, tag) in enumerate(zip(bgs, ['b1_gs', 'b2_gs', 'b3_gs'])): print (tag, b)
            print ('cgs', cgs.reshape(self.Ng, self.Ns))
            print ('cssp', cssp)
            print ('cssm', cssm)
            # print ('cssp', '%.3f' % cssp[0], 'cssm', '%.3f' % cssm[0])
            if self.mag: print ('bmag', bmag)
            if self.baryons: print ('b1i', b1i)
            if self.nnlo: print ('bnnlo', bnnlo)

    def get_bias_arrays(self, bgg, cgg, bgs, cgs, cssp, cssm, loop_only=False):
        ...
        bbgs = np.array([[bs[0], ci, bs[0], bs[2], bs[0], bs[1], bs[3]] for bs, cs in zip(bgs.T, cgs) for ci in cs]) # b1*11, c*ct, b1*13, b3*13, b1*22, b2*22, b4*22
        b1_gs = bgs[0]

        b1, b2, b3, b4 = bgg
        bbgg = np.array([b1**2, 2.*b1*cgg/self.km**2, b1**2, b1*b3, b1**2, b1*b2, b1*b4, b2**2, b2*b4, b4**2]).T

        if self.halofit: # setting shear loops to 0 keeping only the linear (computed with halofit)
            bbssp[:, 1:] *= 0.
            bbssm[:, 1:] *= 0.
        if self.noloop: 
            bbgg[:, 1:] *= 0.
        ...
        return bbssp, bbssm, bbgs, bbgg, b1, b1_gs

    def set_3x2pt(self, bng, bg=None, marg=False, loop_only=False):

        bgg, cgg, bgs, cgs, cssp, cssm, bmag, b1i, bnnlo = self.format_bias(bng, bg=bg, marg=marg)
        bssp, bssm, bgs, bgg, b1, b1_gs = self.get_bias_arrays(bgg, cgg, bgs, cgs, cssp, cssm, loop_only=loop_only)
        ...
        if not loop_only:
            ...
            if self.mag: # we neglect the correlations with the loops
                ...
                bgs_mag = np.array([2.*bmag for _ in range(self.Ns)]).reshape(-1)
                self.Xgs += np.einsum('i,it->it', bgs_mag, self.Ags_mag) 

            if self.rsd and self.mag: 
                self.Xgg += np.einsum('i,it->it', 2.*bmag, self.Agg_rsd_mag)
            ...
            if self.baryons: # adding adiabatic x isocurvature linear correction. b1a : adiabatic linear bias ; b1i : isocurvature linear bias .
                ...
                b1_gs_b1i = np.concatenate([ self.Ns*[b1_gs[i] + b1i[i]] for i in range(self.Ng) ]) 
                self.Xgs += np.einsum('i,it->it', b1_gs_b1i, self.B11[2,:self.Ngs]) 
    ...
    def get_marg(self, bng): 

        bgg, _, _, _, _, _, _, _, bnnlo = self.format_bias(bng, marg=True) # getting bgg (and bnnlo if any)
        b1, _, _, _ = bgg # getting b1 (in gg)
        
        marg = np.zeros(shape=(self.Nmarg, self.Nt*self.Nbin)) # (marginalised parameters, theta bins)
        ...
        for i in np.arange(2*self.Nss+self.Ngs, self.Nbin): marg[i, i*self.Nt:(i+1)*self.Nt] = self.Agg[1,i-(2*self.Nss+self.Ngs)] * 2*b1[i-(2*self.Nss+self.Ngs)]/self.km**2 # gg

        # b3_gg, b3_gs
        for i in range(self.Ng):
            u = self.Nbin + i # id_marg_b3_gg
            ...
            u_gs = u  # id_marg_b3_gs
            if self.extended: u_gs += self.Ng # independent set of (b3) for gs 
            for j in range(self.Ns):
                v = 2*self.Nss + i*self.Ns + j # id_bin_gs
                marg[u_gs, v*self.Nt:(v+1)*self.Nt] = self.Ags[3,i*self.Ns + j]
                if self.nnlo: marg[u_gs, v*self.Nt:(v+1)*self.Nt] += self.Vgs[3,i*self.Ns + j] * bnnlo[2] / self.km**2
